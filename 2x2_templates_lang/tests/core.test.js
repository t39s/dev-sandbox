...
test('source integer outside ±1,000,000 rejected',()=>{let yes=false;try{C.parseExpression('1000001')}catch{yes=true}if(!yes)throw new Error('out-of-range literal accepted')});
...
test('unknown variable name rejected before start',()=>{const x=structuredClone(stock);x.templates=[{id:'unknown',title:'unknown',variables:{a:{min:1,max:9}},expression:'a + c'}];x.levels=[{id:'level-a',title:'x',maxErrors:0,maxSeconds:10,items:[{templateId:'unknown',count:1}],shuffle:false}];if(C.validateSet(x).ok)throw new Error('unknown name accepted')});
...
test('range boundaries are inclusive',async()=>{const p=await C.buildPoolAsync({id:'bounds',title:'bounds',variables:{a:{min:-1,max:1}},expression:'a'});eq(p.length,3);eq(p[0].answer,-1);eq(p[2].answer,1)});
...
test('cross-template duplicate prompt produces warning',async()=>{const x={format:'2x2-templates',version:1,id:'dups',revision:1,title:'dups',templates:[{id:'a',title:'a',expression:'2 + 2'},{id:'b',title:'b',expression:'2 + 2'}],levels:[{id:'l',title:'l',maxErrors:0,maxSeconds:10,items:[{templateId:'a',count:1},{templateId:'b',count:1}],shuffle:false}]};let warning='';await C.generateLevel(x,x.levels[0],{onWarning:w=>warning=w});if(!/одинаковые условия/.test(warning))throw new Error('warning missing')});
test('100k pool preparation can be cancelled',async()=>{const t={id:'large',title:'large',variables:{a:{min:0,max:999},b:{min:0,max:99}},expression:'a * 100 + b'};const ac=new AbortController();let aborted=false;try{await C.buildPoolAsync(t,{signal:ac.signal,onProgress:()=>ac.abort()})}catch(e){aborted=e.name==='AbortError'}if(!aborted)throw new Error('preparation was not cancelled')});
