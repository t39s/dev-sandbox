...
const source = readFileSync(new URL('../../ttScore_0.4.0.html', import.meta.url), 'utf8');
...
test('версия 0.4.0 объявлена, но локальный protocol namespace 0.3.5 сохранён', () => {
  assert.match(source, /<title>ttScore 0\.4\.0<\/title>/);
  assert.match(source, /Версия 0\.4\.0/);
...
test('Team mode является opt-in через teamMatch и не активируется автономно', () => {
  ...
  assert.match(source, /if \(!IS_TEAM_MODE\) return null;/);
});

test('Team adapter загружается лениво только из versioned asset 0.9.0', () => {
  assert.match(source, /team\/assets\/0\.9\.0\/ttscore-team-adapter\.mjs/);
