...
import { prepareArchivedTeamMatch, readArchivedTeamMatch } from "../../team/assets/0.9.0/archive-source.mjs";
import { createTeamMatch } from "../../team/assets/0.9.0/creator.mjs";
import { prepareTransition } from "../../team/assets/0.9.0/editor.mjs";
...
test("архив загружается только из team/matches/<id>/<id>.json без кэша", async () => {
  ...
  const result = await readArchivedTeamMatch(
    "archive-test",
    "https://t39s.github.io/ttscore/team/assets/0.9.0/archive-source.mjs",
    async (url, options) => {
...
test("404 означает отсутствие архивной копии, остальные HTTP-ошибки не маскируются", async () => {
  assert.equal(await readArchivedTeamMatch("archive-test", "https://example.test/team/assets/0.9.0/archive-source.mjs", async () => ({ ok: false, status: 404 })), null);
  await assert.rejects(
    readArchivedTeamMatch("archive-test", "https://example.test/team/assets/0.9.0/archive-source.mjs", async () => ({ ok: false, status: 503 })),
    /HTTP 503/
  );
});
