...
test("HTML подключает общие assets версии 0.9.0", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  assert.match(html, /\.\/assets\/0\.9\.0\/styles\.css/);
  assert.match(html, /\.\/assets\/0\.9\.0\/app\.mjs/);
...
test("переименование matches-source выполнено полностью", () => {
  assert.equal(existsSync(new URL("team/assets/0.9.0/matches-source.mjs", root)), true);
  assert.equal(existsSync(new URL("team/assets/0.9.0/meeting-source.mjs", root)), false);
  assert.match(read("team/assets/0.9.0/app.mjs"), /\.\/matches-source\.mjs/);
  assert.doesNotMatch(read("team/assets/0.9.0/app.mjs"), /meeting-source/);
});

test("Firebase остаётся оперативным источником, GitHub JSON используется только как public read-only fallback", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  const app = read("team/assets/0.9.0/app.mjs");
  const firebase = read("team/assets/0.9.0/firebase-source.mjs");
  const archive = read("team/assets/0.9.0/archive-source.mjs");
...
test("относительные reportUrl по-прежнему разрешаются от team/matches/<id>/", () => {
  const source = read("team/assets/0.9.0/matches-source.mjs");
  assert.match(source, /matches\/\$\{id\}\//);
  assert.match(source, /teamMatchLinkedResourceUrl/);
  assert.match(read("team/assets/0.9.0/app.mjs"), /publishedLinkHref/);
});

test("данные примера отсутствуют в общем исходном коде", () => {
  const source = [
    read("team/ttscore_team_0.9.0.html"),
    read("team/assets/0.9.0/app.mjs"),
    read("team/assets/0.9.0/creator.mjs"),
    read("team/assets/0.9.0/editor.mjs"),
    read("team/assets/0.9.0/file-save.mjs"),
    read("team/assets/0.9.0/firebase-source.mjs"),
    read("team/assets/0.9.0/archive-source.mjs"),
    read("team/assets/0.9.0/model.mjs"),
    read("team/assets/0.9.0/matches-source.mjs"),
    read("team/assets/0.9.0/ui-state.mjs"),
    read("team/assets/0.9.0/ttscore-integration.mjs"),
    read("team/assets/0.9.0/styles.css")
  ].join("\n");
...
test("публичная страница не предлагает переход в редактор", () => {
  ...
  assert.match(read("team/assets/0.9.0/app.mjs"), /request\.mode !== "edit"/);
});
...
test("одно адаптивное сохранение выбирает picker, Web Share и только затем Blob", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  const app = read("team/assets/0.9.0/app.mjs");
  const saver = read("team/assets/0.9.0/file-save.mjs");
...
test("идущая встреча редактирует сведения и planned-порядок единым preview", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  const app = read("team/assets/0.9.0/app.mjs");
  const editor = read("team/assets/0.9.0/editor.mjs");
  for (const id of [
...
test("v0.9.0 сворачивает все рабочие разделы edit кроме завершения текущей встречи", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  const app = read("team/assets/0.9.0/app.mjs");
...
test("v0.9.0 показывает операционные Live-табло и Live-отчёт и использует названия команд в итоговом счёте", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  const app = read("team/assets/0.9.0/app.mjs");
  const integration = read("team/assets/0.9.0/ttscore-integration.mjs");
...
test("редактор выделяет одну встречу нажатием и после перемещения", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  const app = read("team/assets/0.9.0/app.mjs");
  const styles = read("team/assets/0.9.0/styles.css");
...
test("режим create подключён без GitHub API", () => {
  ...
  assert.match(read("team/ttscore_team_0.9.0.html"), /id="creator-id-warning"/);
...
test("создатель загружает локальный JSON без GitHub API и миграции schemaVersion 3", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  const app = read("team/assets/0.9.0/app.mjs");
  const creator = read("team/assets/0.9.0/creator.mjs");
...
test("v0.9.0 сохраняет локальную интеграцию ttScore без backend", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  const app = read("team/assets/0.9.0/app.mjs");
  const integration = read("team/assets/0.9.0/ttscore-integration.mjs");
  assert.equal(existsSync(new URL("team/assets/0.9.0/ttscore-integration.mjs", root)), true);
  for (const id of [
...
test("v0.9.0 не создаёт и не редактирует liveUrl в командном JSON", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  const app = read("team/assets/0.9.0/app.mjs");
  const creator = read("team/assets/0.9.0/creator.mjs");
  const editor = read("team/assets/0.9.0/editor.mjs");
...
test("v0.9.0 публикует Live-отчёт и Live-табло текущей встречи из верхнего уровня JSON", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  const app = read("team/assets/0.9.0/app.mjs");
  const creator = read("team/assets/0.9.0/creator.mjs");
  const editor = read("team/assets/0.9.0/editor.mjs");
...
test("v0.9.0 использует Firebase как опубликованный источник и realtime для public view", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  const app = read("team/assets/0.9.0/app.mjs");
  const firebase = read("team/assets/0.9.0/firebase-source.mjs");
  for (const id of [
...
test("v0.9.0 Firebase-editor подписан на внешние realtime updates и защищает локальный draft", () => {
  const app = read("team/assets/0.9.0/app.mjs");
  assert.match(app, /startFirebaseRealtimeEditor/);
  assert.match(app, /applyRealtimeEditorData/);
...
test("v0.9.0 после запуска следующей ttScore-встречи не перезаписывает ручную корректировку pending-результата событиями live", () => {
  const app = read("team/assets/0.9.0/app.mjs");
...
test("v0.9.0 автоматически публикует только подтверждённые ttScore-переходы и операционные Live-ссылки", () => {
  const app = read("team/assets/0.9.0/app.mjs");
  const integration = read("team/assets/0.9.0/ttscore-integration.mjs");
  const editor = read("team/assets/0.9.0/editor.mjs");
  const contract = read("team/assets/0.9.0/team-integration-contract.mjs");
...
test("v0.9.0 не записывает состояние ttScore и не изменяет его локальный протокол", () => {
  const app = read("team/assets/0.9.0/app.mjs");
  const integration = read("team/assets/0.9.0/ttscore-integration.mjs");
...
test("v0.9.0 даёт Firebase-editor действие запуска текущей пары в ttScore Team mode", () => {
  const html = read("team/ttscore_team_0.9.0.html");
  const app = read("team/assets/0.9.0/app.mjs");
  const adapter = read("team/assets/0.9.0/ttscore-team-adapter.mjs");
  const contract = read("team/assets/0.9.0/team-integration-contract.mjs");
  assert.match(html, /id="editor-open-ttscore"/);
  assert.match(app, /new URL\("\.\.\/ttScore_0\.4\.0\.html", location\.href\)/);
...
test("deployable bundle сохраняет общий корень ttScore 0.4.0 и Team assets 0.9.0", () => {
  assert.equal(existsSync(new URL("ttScore_0.4.0.html", root)), true);
  assert.equal(existsSync(new URL("team/assets/0.9.0/ttscore-team-adapter.mjs", root)), true);
  const app = read("team/assets/0.9.0/app.mjs");
  const ttScore = read("ttScore_0.4.0.html");
  assert.match(app, /new URL\("\.\.\/ttScore_0\.4\.0\.html", location\.href\)/);
  assert.match(ttScore, /new URL\("\.\/team\/assets\/0\.9\.0\/ttscore-team-adapter\.mjs", location\.href\)/);
});
